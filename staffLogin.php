<?php
  ...
  function retrieveStaffnameId($con, $email) {

    // Prepare and execute the SQL query
    $query = "SELECT `staffname`, `staff_id` FROM `staff` WHERE `email` = ?";
    ...
    if ($result && $result->num_rows > 0) {
        // Fetch the staffname from the result
        $row = $result->fetch_assoc();
        $staffname = $row['staffname'];
        $staffId = $row['staff_id'];

        // Set the staffname in the session variable
...
<html lang="en">
  <head>
    <title>Login to FAZADA STAFF</title>
    <meta charset="UTF-8">
    ...
    if (isset($_POST['email'])){
      ...
      $query = "SELECT * 
        ...
        if ($rows==1) {
          ...
          echo "<div class='form'>
          <h3>Entered staffname/password is incorrect. Please try again.</h3>
          <br/>Click here to <a href='staffLogin.php'>Staff Login</a></div>";
        }
    } else {
      ...
      <div class="container-login100">
        <div class="wrap-login100 p-t-50 p-b-90">
          <form class="login100-form validate-form flex-sb flex-w" action="" method="post" name="login">
            <span class="login100-form-title p-b-51">STAFF in FAZADA.com!</span>
            ...
            <div class="container-login100-form-btn m-t-17"><button class="login100-form-btn" name="submit" type="submit" value="Login">Login to STAFF FAZADA</button></div>
          </form>
        </div>
      </div>
    </div>
